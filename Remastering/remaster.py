...
parser.add_argument('--disable_colorization', action='store_true',
                    default=False, help='Remaster without colorization')
parser.add_argument('--gpu',       action='store_true',
                    default=False, help='Use GPU')
parser.add_argument('--mindim',     type=int,   default='320',
...
if not opt.disable_colorization:
    if opt.reference_dir != 'none':
        ...
        reference_files = []
        for ext in ext_list:
            reference_files += glob.glob(opt.reference_dir +
                                         '/*.'+ext, recursive=True)
        aspect_mean = 0
        minedge_dim = 256
        refs = []
        ...
        refimgs = torch.FloatTensor(
            len(reference_files), 3, target_h, target_w)
        ...
        refimgs = refimgs.view(1, refimgs.size(0), refimgs.size(
            1), refimgs.size(2), refimgs.size(3)).to(device)

# Load video
cap = cv2.VideoCapture(opt.input)


# here made some changes look according to it
    ...
    while True:
        ...
        for i in range(proc_g):
            ...
            if nchannels == 1 or not opt.disable_colorization:
                ...
                frame_l = torch.from_numpy(frame_l).view(
                    frame_l.shape[0], frame_l.shape[1], 1)
                frame_l = frame_l.permute(2, 0, 1).float()  # HWC to CHW
                frame_l /= 255.
                frame_l = frame_l.view(1, frame_l.size(
                    0), 1, frame_l.size(1), frame_l.size(2))
            elif nchannels == 3:
                ...
                frame_l = frame_l.view(1, frame_l.size(
                    0), 1, frame_l.size(1), frame_l.size(2))
                frame_ab = frame_ab.view(1, frame_ab.size(
                    0), 1, frame_ab.size(1), frame_ab.size(2))
        ...
        if opt.disable_colorization:
            for i in range(proc_g):
                ...
                if nchannels == 3:
                    ...
                    out_ab = gtC[0, :, i].cpu()
                    out = torch.cat((out_l, out_ab), dim=0).detach(
                    ).numpy().transpose((1, 2, 0))
                    out = Image.fromarray(
                        np.uint8(utils.convertLAB2RGB(out)*255))
                    out.save(outputdir_out+'%07d.png' % (index))
                else:
                    save_image(output_l.detach()[
                               0, :, i], outputdir_out+'%07d.png' % (index), nrow=1)
        # Perform colorization
            ...
            for i in range(proc_g):
                ...
                out_c = output_ab[0, :, i, :, :]
                output = torch.cat(
                    (out_l, out_c), dim=0).numpy().transpose((1, 2, 0))
